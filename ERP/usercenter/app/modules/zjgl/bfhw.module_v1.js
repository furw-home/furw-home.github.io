	...
	djclbModule.controller('djclbCtrl', ['$scope', 'utilService', 'uiService',
		function($scope, utilService, uiService) {
			...
			$scope.plzj = function() {
				...
				//清除上传文件
				$("#file-list").html("");
				var _osskey_bfhw = window.sessionStorage.getItem("_osskey_bfhw");
				if(_osskey_bfhw == null) {} else {
					sessionStorage.removeItem("_osskey_bfhw"); //移除
				};
			...
			$scope.plzj_save = function(project) {
				...
				var _osskey_bfhw = window.sessionStorage.getItem("_osskey_bfhw");
				if(null == _osskey_bfhw) {} else {
					_osskey_bfhw = JSON.parse(_osskey_bfhw);
					project.filekey = _osskey_bfhw[0].qcfilekey;
					project.filename = _osskey_bfhw[0].qcfilename;
				}
