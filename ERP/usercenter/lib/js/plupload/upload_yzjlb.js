		...
		var uploader = new plupload.Uploader({
			...
			init: {
				...
				FilesAdded: function(up, files) {
					...
					plupload.each(files, function(file) {
						if(!up.settings.multi_selection) // 仅可以选择一个文件时，先清空文件列表，再拼接本次选择的文件信息
							document.getElementById('file-list').innerHTML = '';
						document.getElementById('file-list').innerHTML += '<div id="' + file.id + '" class="rel"><span class="filename" title="' + file.name + '">' + file.name + '</span> (' + plupload.formatSize(file.size) + ')<b> </b><span class="remove_files icon-remove"></span>' +
				...
				BeforeUpload: function(up, file) {
					...
					window.sessionStorage.setItem("_osskey_yzjlb", _attachmentss);
