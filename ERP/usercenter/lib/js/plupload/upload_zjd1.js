...
(function($) {
	$.fn.Pluploader = function(options, cb) {
		var _this = this;
		accessid = ''
		accesskey = ''
		host = ''
		...
		var uploader = new plupload.Uploader({
			...
			init: {
				...
				FilesAdded: function(up, files) {
					...
					plupload.each(files, function(file) {
						if(!up.settings.multi_selection) // 仅可以选择一个文件时，先清空文件列表，再拼接本次选择的文件信息
							document.getElementById('file-list').innerHTML = '';
						document.getElementById('file-list').innerHTML += '<div id="' + file.id + '">' + file.name + ' (' + plupload.formatSize(file.size) + ')<b> </b><span class="remove_files icon-remove"></span>' +
				...
				BeforeUpload: function(up, file) {
					...
					osskey = key + '/'; // 模块名
					//osskey += prefilename.substr(0, 8) + '/'; // 8位日期文件夹
					osskey += prefilename + get_suffix(file.name); //file.name; // 文件名
					var osskeyfileName = file.name;

					var attachmentsStr = {};
					attachmentsStr.qcfilekey = osskey;
					attachmentsStr.qcfilename = osskeyfileName;
					attachments.push(attachmentsStr);
					var attachmentss = JSON.stringify(attachments);
					window.sessionStorage.setItem("osskey", attachmentss);

					set_upload_param(up, osskey, false);
				},
				...
				FileUploaded: function(up, file, info) {
					...
					if(info.status == 200) {
						if(!_osskeyArr.contains(osskey))
							_osskeyArr.push(osskey);
